...
const GOLD = "#1FA84A"; // primary action / positive money state
...
const THEME_VARS = {
  light: {
    "--pr-ink": "#12181B", "--pr-ink-soft": "#16211A", "--pr-paper": "#F5F6F0", "--pr-paper-dim": "#FFFFFF",
    "--pr-line": "#E7E9E1", "--pr-muted": "#707E74", "--pr-input-bg": "#F1F3EC", "--pr-active-tint": "#E6F6EA",
    "--pr-danger-bg": "#FDEDEB", "--pr-nav-muted": "#536159", "--pr-primary-disabled-bg": "#DCE8DF", "--pr-primary-disabled-ink": "#5C6B62",
  },
  dark: {
    "--pr-ink": "#F1F4F0", "--pr-ink-soft": "#E4E9E1", "--pr-paper": "#0E1512", "--pr-paper-dim": "#182019",
    "--pr-line": "#2B342E", "--pr-muted": "#8FA398", "--pr-input-bg": "#1E2620", "--pr-active-tint": "#16341F",
...
const R_MD = 14;
const R_LG = 20;
const SHADOW_CARD = "0 8px 24px -20px rgba(11,35,70,0.28)";
const SHADOW_BTN = "0 10px 24px -14px rgba(31,168,74,0.34)";
const GOLD_GRADIENT = "linear-gradient(145deg, #20B451 0%, #168E3A 100%)";
const HERO_GRADIENT = "linear-gradient(145deg, #0B2346 0%, #123A32 58%, #1FA84A 140%)";
...
button:focus-visible, input:focus-visible, textarea:focus-visible, select:focus-visible { outline: 2px solid rgba(31,168,74,0.55); outline-offset: 2px; }
...
async function deriveStorageKey(pin) { try { const salt=new TextEncoder().encode("PocketRule-storage-v1"); const base=await crypto.subtle.importKey("raw",new TextEncoder().encode(pin),"PBKDF2",false,["deriveKey"]); return await crypto.subtle.deriveKey({name:"PBKDF2",salt,iterations:210000,hash:"SHA-256"},base,{name:"AES-GCM",length:256},false,["encrypt","decrypt"]); } catch { return null; } }
async function encryptJson(value,pin) { try { const key=await deriveStorageKey(pin); if(!key)return null; const iv=new Uint8Array(12); crypto.getRandomValues(iv); const cipher=await crypto.subtle.encrypt({name:"AES-GCM",iv},key,new TextEncoder().encode(JSON.stringify(value))); return JSON.stringify({version:SECURITY_VERSION,iv:bytesToHex(iv),data:bytesToHex(cipher)}); } catch { return null; } }
async function encryptBackupWithPassword(value,password) {
  try {
    ...
    const key = await crypto.subtle.deriveKey({name:"PBKDF2",salt,iterations:210000,hash:"SHA-256"},base,{name:"AES-GCM",length:256},false,["encrypt","decrypt"]);
    const cipher = await crypto.subtle.encrypt({name:"AES-GCM",iv},key,new TextEncoder().encode(JSON.stringify(value)));
    return JSON.stringify({version:2,kdf:"PBKDF2-SHA256",iterations:210000,salt:bytesToHex(salt),iv:bytesToHex(iv),ciphertext:bytesToHex(cipher)});
  } catch { return null; }
}
async function decryptBackupWithPassword(payload,password) {
  try {
    ...
    const base=await crypto.subtle.importKey("raw",new TextEncoder().encode(password),"PBKDF2",false,["deriveKey"]);
    const key=await crypto.subtle.deriveKey({name:"PBKDF2",salt,iterations:x.iterations||210000,hash:"SHA-256"},base,{name:"AES-GCM",length:256},false,["encrypt","decrypt"]);
    const plain=await crypto.subtle.decrypt({name:"AES-GCM",iv},key,cipher);
    return JSON.parse(new TextDecoder().decode(plain));
  } catch { return null; }
}
async function decryptJson(payload,pin) { try { const x=typeof payload==="string"?JSON.parse(payload):payload; if(!x||x.version!==SECURITY_VERSION)return null; const key=await deriveStorageKey(pin),iv=hexToBytes(x.iv),cipher=hexToBytes(x.data); if(!key||!iv||!cipher)return null; const plain=await crypto.subtle.decrypt({name:"AES-GCM",iv},key,cipher); return JSON.parse(new TextDecoder().decode(plain)); } catch { return null; } }
...
function PremiumSectionLabel({ children }) {
  return <p style={{ margin: "0 0 8px 2px", fontFamily: "Inter, sans-serif", fontSize: 10, lineHeight: 1.2, fontWeight: 800, letterSpacing: 0.7, textTransform: "uppercase", color: MUTED }}>{children}</p>;
}
...
function normalizeSettings(rawSettings, baseSettings) {
  ...
  const rawPin = s.pin;
  const pin = typeof rawPin === "string" && /^\d{4}$/.test(rawPin) ? rawPin : null;

  const appearance = ["light", "dark", "system"].includes(s.appearance) ? s.appearance : baseSettings.appearance;
  const reminderFrequency = ["off", "daily", "weekly", "monthly"].includes(s.reminderFrequency)
    ? s.reminderFrequency
  ...
  return {
    ...baseSettings,
    ...s,
    currency: typeof s.currency === "string" && s.currency.trim() ? s.currency.trim().toUpperCase() : baseSettings.currency,
    reminderFrequency,
    reminderDay,
    reminderDayOfMonth,
    reminderHour,
      ...
      <div style={{ position: "absolute", top: 0, right: 0, bottom: 0, display: "flex" }}>
        {actions.map((a, i) => {
          ...
          return (
            ...
            >
              <Icon size={17} />
              <span style={{ fontFamily: "Inter, sans-serif", fontSize: 10, fontWeight: 700 }}>{a.label}</span>
            </button>
          );
        })}
      </div>
...
const labelStyle = {
  fontFamily: "Inter, sans-serif", fontSize: 11.5, fontWeight: 700, letterSpacing: 0.4,
  ...
  return (
    <div style={{ minHeight: "100%", width: "100%", boxSizing: "border-box", display: "flex", alignItems: "center", justifyContent: "center", padding: "28px 18px", background: dark ? "#0A100D" : "#F5F6F0" }}>
      <div style={{ width: "100%", maxWidth: 560, boxSizing: "border-box", background: dark ? "#182019" : "#FFFFFF", border: `1px solid ${dark ? "#2B342E" : "#E7E9E1"}`, borderRadius: 30, padding: "34px 28px 28px", boxShadow: "0 24px 60px rgba(0,0,0,0.28)", textAlign: "center" }}>
        ...
        <div style={{ marginTop: 24, display: "flex", justifyContent: "center", alignItems: "center", gap: 8, color: dark ? "rgba(241,244,240,0.42)" : "#707E74", fontFamily: '"Roboto Mono", monospace', fontSize: 10.5, letterSpacing: "0.5px" }}>
...
/* ---------------------------------------------------------
   ONBOARDING
--------------------------------------------------------- */
function Onboarding({ reminder, setReminder, onFinish, rules, selectedRuleId, onSelectRule, currency, onCurrencyChange, onAddCurrency }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <div style={{ display: "flex", justifyContent: "center", gap: 6, paddingTop: 12 }}>
        {[1, 2].map((n) => (
          <div key={n} style={{ width: n === step ? 24 : 7, height: 7, borderRadius: 999, background: n === step ? GOLD : LINE, transition: "all 180ms ease" }} />
        ))}
      </div>
        ...
        {step === 1 ? (
          <div style={{ textAlign: "center" }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 7 }}>
              <LogoMark size={26} />
              <span style={{ fontFamily: "Sora, sans-serif", fontWeight: 800, fontSize: 18, color: INK }}>Pocket<span style={{ color: BRAND_ORANGE }}>Rule</span></span>
            </div>

            <p style={{ fontFamily: "Roboto, sans-serif", fontWeight: 600, fontSize: 11.5, color: GOLD, margin: "28px 0 8px" }}>01 — THE IDEA</p>
            <h1 style={{ fontFamily: "Sora, sans-serif", fontWeight: 800, fontSize: 29, letterSpacing: "-0.035em", color: INK, margin: 0, lineHeight: 1.08 }}>
              Turn any money<br />into a <span style={{ color: GOLD }}>plan.</span>
            </h1>
            <p style={{ fontFamily: "Inter, sans-serif", fontSize: 14, color: MUTED, margin: "14px auto 0", lineHeight: 1.5, maxWidth: 310 }}>
              Set your rules. Enter any amount. PocketRule gives every amount a job.
            </p>

            <div style={{ margin: "20px auto 0", maxWidth: 310, background: PAPER_DIM, border: `1px solid ${LINE}`, borderRadius: 16, padding: 12, boxShadow: SHADOW_CARD }}>
              <p style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, color: MUTED, margin: "0 0 7px", textTransform: "uppercase", letterSpacing: 0.5, fontWeight: 800, textAlign: "left" }}>Your currency</p>
              <button
                onClick={() => setShowCurrencySheet(true)}
                aria-label="Choose currency"
                style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, padding: "11px 12px", borderRadius: 12, border: `1px solid ${LINE}`, background: INPUT_BG, color: INK, cursor: "pointer", fontFamily: "Inter, sans-serif", fontWeight: 700, fontSize: 13 }}
              >
                <span>{currencyLabel(CURRENCIES.find((c) => c.code === currency) || CURRENCIES[0])}</span>
                <span style={{ display: "inline-flex", alignItems: "center", gap: 4, color: GOLD }}>Change <ChevronDown size={15} /></span>
              </button>
            </div>

            <div style={{ marginTop: 24, background: GREEN_CARD_GRADIENT, borderRadius: 22, padding: 18, boxShadow: SHADOW_BTN, color: "#fff", textAlign: "left" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                <span style={{ fontFamily: "Inter, sans-serif", fontSize: 11, fontWeight: 700, opacity: 0.72, textTransform: "uppercase", letterSpacing: 0.5 }}>A simple plan</span>
                <span style={{ fontFamily: "Roboto, sans-serif", fontSize: 12, fontWeight: 700, opacity: 0.85 }}>Any amount</span>
              </div>
              <div style={{ display: "flex", height: 64, borderRadius: 14, overflow: "hidden", gap: 2 }}>
                {[30,25,15,20,10].map((pct, i) => (
                  <div key={i} style={{ width: `${pct}%`, background: SEGMENT_COLORS[i], display: "flex", alignItems: "flex-end", justifyContent: "center", paddingBottom: 7, boxSizing: "border-box" }}>
                    <span style={{ fontFamily: "Roboto, sans-serif", fontSize: 9.5, fontWeight: 700, color: "#fff" }}>{pct}%</span>
                  </div>
                ))}
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 12, fontFamily: "Inter, sans-serif", fontSize: 10.5, opacity: 0.8 }}>
                <span>Food</span><span>Rent</span><span>Transport</span><span>Savings</span><span>Business</span>
              </div>
            </div>

            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginTop: 20, color: MUTED }}>
              <Shield size={15} color={GOLD} />
              <span style={{ fontFamily: "Inter, sans-serif", fontSize: 11.5 }}>Private by default. No bank access. Your money, your rules.</span>
            </div>
          </div>
        ) : (
          <div>
            <p style={{ fontFamily: "Roboto, sans-serif", fontWeight: 600, fontSize: 11.5, color: GOLD, margin: "10px 0 8px", textAlign: "center" }}>02 — YOUR STARTING RULE</p>
            <h1 style={{ fontFamily: "Sora, sans-serif", fontWeight: 800, fontSize: 25, letterSpacing: "-0.025em", color: INK, margin: 0, lineHeight: 1.15, textAlign: "center" }}>
              Start with a Rule.
            </h1>
            <p style={{ fontFamily: "Inter, sans-serif", fontSize: 13, color: MUTED, margin: "8px auto 18px", lineHeight: 1.45, textAlign: "center", maxWidth: 300 }}>
              Choose a Rule, then edit it or create your own anytime.
            </p>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {starterRules.map((r) => {
                ...
                return (
                  <button key={r.id} onClick={() => onSelectRule(r.id)} style={{ width: "100%", textAlign: "left", display: "flex", alignItems: "center", gap: 12, padding: "13px 14px", borderRadius: 15, border: `1.5px solid ${active ? GOLD : LINE}`, background: active ? ACTIVE_TINT : PAPER_DIM, boxShadow: active ? "0 8px 20px -14px rgba(31,168,74,0.45)" : SHADOW_CARD, cursor: "pointer" }}>
                    <div style={{ width: 38, height: 38, borderRadius: 10, background: active ? "#FFFFFF" : INPUT_BG, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><RuleIcon rule={r} size={19} color={active ? GOLD : INK} /></div>
                    <div style={{ minWidth: 0, flex: 1 }}>
                      <p style={{ fontFamily: "Inter, sans-serif", fontWeight: 800, fontSize: 13.5, color: INK, margin: 0 }}>{r.name}</p>
                      <p style={{ fontFamily: "Inter, sans-serif", fontSize: 11, color: MUTED, margin: "3px 0 0", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                        {r.categories.map((c) => `${c.name} ${c.pct}%`).join(" · ")}
                      </p>
                    </div>
                    {active && <Check size={18} color={GOLD} strokeWidth={3} />}
                  </button>
                );
              })}
            </div>

            <div style={{ marginTop: 16, background: PAPER_DIM, border: `1px solid ${LINE}`, borderRadius: 16, padding: 14, boxShadow: SHADOW_CARD }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                ...
                <div style={{ minWidth: 0, flex: 1 }}>
                  <p style={{ fontFamily: "Inter, sans-serif", fontWeight: 800, fontSize: 13, color: INK, margin: 0 }}>Stay on track with reminders</p>
                  <p style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, color: MUTED, margin: "3px 0 0", lineHeight: 1.35 }}>Get a gentle reminder to review your money plan.</p>
                </div>
              </div>
              ...
              {reminder.frequency !== "off" && (
                <p style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, color: MUTED, margin: "9px 2px 0", lineHeight: 1.35 }}>
                  You can change this anytime in Settings. PocketRule will ask for notification permission when you finish setup.
                </p>
              )}
              {reminder.frequency === "off" && (
                <p style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, color: MUTED, margin: "9px 2px 0", lineHeight: 1.35 }}>
                  You can turn reminders on later from Settings.
                </p>
              )}
            </div>
          </div>
      ...
      <div style={{ padding: "14px 20px 18px", borderTop: `1px solid ${LINE}`, flexShrink: 0 }}>
        {step === 1 ? (
          <PrimaryButton icon={ArrowRight} onClick={() => setStep(2)}>Continue</PrimaryButton>
        ) : (
          <PrimaryButton icon={Check} onClick={finish}>Start planning</PrimaryButton>
        )}
        {step === 2 && (
          <button onClick={() => setStep(1)} style={{ width: "100%", marginTop: 8, background: "none", border: "none", color: MUTED, fontFamily: "Inter, sans-serif", fontWeight: 600, fontSize: 11.5, cursor: "pointer" }}>Back</button>
        )}
      </div>
    </div>
  );
...
--------------------------------------------------------- */
function PlanTracker({ plan, currency, onAddExpense, onEditExpense, onDeleteExpense, onFinish, isDark, resourcePrompt, onOpenResource }) {
  ...
  return (
    <section style={{ marginTop: 8, paddingBottom: 6 }}>
      <div className="pr-rise" style={{
        background: PAPER_DIM, border: `1px solid ${LINE}`, borderRadius: 22, padding: 14,
        boxShadow: "0 14px 34px -24px rgba(18,24,27,0.42)",
      }}>
        <div style={{ textAlign: "center", minWidth: 0 }}>
          <div style={{ width: 42, height: 42, borderRadius: 13, background: ACTIVE_TINT, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 9px" }}>
            <PiggyBank size={20} color={GOLD} strokeWidth={2.5} />
          </div>
          <p style={{ fontFamily: "Inter, sans-serif", fontSize: 9.5, color: MUTED, margin: 0, textTransform: "uppercase", letterSpacing: 0.9, fontWeight: 800 }}>Active plan</p>
          <p style={{ fontFamily: "Sora, sans-serif", fontWeight: 800, fontSize: 16, color: INK, margin: "3px auto 0", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: "100%" }}>{plan.name || "Current plan"}</p>
          <p style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, color: MUTED, margin: "3px 0 0" }}>{new Date(plan.date).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" })}</p>
        </div>
        <div style={{
          position: "sticky", bottom: 0, zIndex: 12, margin: "12px -15px 0", padding: "8px 15px calc(8px + env(safe-area-inset-bottom))",
          background: `linear-gradient(to bottom, transparent, ${PAPER} 24%)`, borderTop: `1px solid ${LINE}`
        }}>
          <button className="pr-press" onClick={() => setConfirmFinish(true)} style={{
            width: "100%", border: `1.5px solid ${GOLD}`, background: PAPER_DIM, color: GOLD, borderRadius: 12,
            padding: "10px 12px", fontFamily: "Inter, sans-serif", fontWeight: 800, fontSize: 12,
            cursor: "pointer", transition: "transform 120ms ease, background 120ms ease", boxShadow: SHADOW_CARD
          }}>
            <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 7 }}><Check size={15} strokeWidth={2.8} /> I've finished this plan</span>
          </button>
        </div>
        <p style={{ fontFamily: "Inter, sans-serif", fontSize: 10, color: MUTED, margin: "5px 2px 0", textAlign: "center" }}>Move this plan to Completed Plans. You can still review it later.</p>
        <div style={{ textAlign: "center", marginTop: 12 }}>
          <span style={{ fontFamily: "Inter, sans-serif", fontSize: 9.5, color: MUTED, fontWeight: 700, letterSpacing: .45, textTransform: "uppercase" }}>Plan total</span>
          <span className="pr-money" style={{ display: "block", marginTop: 2, fontSize: 12.5, color: MUTED, fontWeight: 700 }}>{formatMoney(totalBudget, currency)}</span>
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "1.18fr 1fr", gap: 8, marginTop: 9 }}>
          <div style={{ background: GOLD_GRADIENT, borderRadius: 16, padding: "14px 13px", color: "#fff", boxShadow: SHADOW_BTN, textAlign: "center" }}>
            <p style={{ fontFamily: "Inter, sans-serif", fontSize: 9.5, margin: 0, opacity: 0.78, textTransform: "uppercase", fontWeight: 800, letterSpacing: 0.3 }}>Remaining</p>
            <p className="pr-money" style={{ fontFamily: "Roboto, sans-serif", fontWeight: 700, fontSize: 21, margin: "5px 0 0", letterSpacing: "-0.04em" }}>{formatMoney(totalRemaining, currency)}</p>
            <p style={{ fontFamily: "Inter, sans-serif", fontSize: 9.5, margin: "5px 0 0", opacity: 0.72 }}>of {formatMoney(totalBudget, currency)}</p>
          </div>
          <div style={{ background: INPUT_BG, borderRadius: 16, padding: "14px 13px", textAlign: "center" }}>
            <p style={{ fontFamily: "Inter, sans-serif", fontSize: 9.5, margin: 0, color: MUTED, textTransform: "uppercase", fontWeight: 800, letterSpacing: 0.3 }}>Spent</p>
            <p style={{ fontFamily: "Roboto, sans-serif", fontWeight: 800, fontSize: 15, margin: "9px 0 0", color: INK }}>{formatMoney(totalSpent, currency)}</p>
          </div>
        </div>

        <div style={{ marginTop: 13 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
            <span style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, color: MUTED }}>Plan progress</span>
          </div>
          <div style={{ height: 7, background: LINE, borderRadius: 999, overflow: "hidden" }}>
            <div style={{ width: `${progress}%`, height: "100%", background: progress >= 100 ? CORAL : GOLD, borderRadius: 999, transition: "width 220ms ease" }} />
          </div>
        </div>
      </div>

      {resourcePrompt && (
        <div className="pr-rise" style={{ marginTop: 12, background: ACTIVE_TINT, border: `1px solid ${LINE}`, borderRadius: 16, padding: "12px 13px" }}>
          <p style={{ margin: 0, fontFamily: "Inter, sans-serif", fontSize: 10, color: GOLD, fontWeight: 800, letterSpacing: .7 }}>A QUICK TIP FOR YOUR PLAN</p>
          <p style={{ margin: "4px 0 3px", fontFamily: "Sora, sans-serif", fontSize: 14.5, lineHeight: 1.25, fontWeight: 800, color: INK }}>{resourcePrompt.title}</p>
          <p style={{ margin: 0, fontFamily: "Inter, sans-serif", fontSize: 12.5, lineHeight: 1.4, color: MUTED }}>{resourcePrompt.message}</p>
      ...
      <div style={{ margin: "17px 2px 10px", textAlign: "center" }}>
        <p style={{ fontFamily: "Inter, sans-serif", fontSize: 9.5, fontWeight: 800, letterSpacing: .8, textTransform: "uppercase", color: MUTED, margin: 0 }}>
          Your categories
        </p>
        <p style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, color: MUTED, margin: "3px 0 0" }}>
          Tap a category to record spending.
        </p>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
        {displayCategories.map((c, index) => {
          ...
          const planPct = totalBudget > 0 ? Math.round((c.budget / totalBudget) * 100) : 0;
          return (
            <button className="pr-press" key={c.id} onClick={() => setSelectedCategoryId(c.id)} style={{
              ...
              borderRadius: 16, padding: "11px 12px 9px", background: PAPER_DIM, cursor: "pointer",
              color: INK, boxShadow: "none",
              transition: "transform 150ms cubic-bezier(0.2,0.8,0.2,1), opacity 150ms ease",
              animation: `pr-rise 180ms ease ${Math.min(index * 18, 120)}ms both`,
            }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ width: 38, height: 38, borderRadius: 11, background: over ? DANGER_BG : ACTIVE_TINT, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <CategoryIcon name={c.name} size={18} color={GOLD} />
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                    <span style={{ fontFamily: "Inter, sans-serif", fontWeight: 800, fontSize: 13, color: INK }}>
                      {c.name}
                    </span>
                    <span style={{ fontFamily: "Roboto, sans-serif", fontWeight: 800, fontSize: 13, color: over ? CORAL : INK, flexShrink: 0 }}>
                      {formatMoney(c.budget, currency)}
                    </span>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 6, marginTop: 3 }}>
                    <span style={{ fontFamily: "Inter, sans-serif", fontSize: 10, color: MUTED }}>
                      {planPct}% of plan · {formatMoney(c.spent, currency)} spent
                    </span>
                    <span style={{ fontFamily: "Inter, sans-serif", fontSize: 10, color: over ? CORAL : GOLD, fontWeight: 700, flexShrink: 0 }}>
                      {over ? `${formatMoney(c.spent - c.budget, currency)} over` : complete ? "All spent" : `${formatMoney(Math.max(0, remaining), currency)} left`}
                    </span>
                  </div>
                </div>
              </div>
              <div style={{ height: 4, background: LINE, borderRadius: 999, overflow: "hidden", marginTop: 9 }}>
      ...
      {successToast && (
        <div
          ...
          style={{
            ...
            zIndex: 100, width: "calc(100% - 48px)", maxWidth: 340,
            background: isDark ? "#F1F4F0" : "#12181B",
        ...
        >
          ...
          <div style={{ minWidth: 0 }}>
            <p style={{ fontFamily: "Inter, sans-serif", fontWeight: 800, fontSize: 11.5, margin: 0 }}>{successToast.deleted ? "Spending deleted" : successToast.edited ? "Spending updated" : "Spending recorded"}</p>
            <p style={{ fontFamily: "Inter, sans-serif", fontSize: 10, color: isDark ? "rgba(18,24,27,0.68)" : "rgba(255,255,255,0.72)", margin: "2px 0 0", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 80, display: "flex", alignItems: "flex-end" }}>
      ...
      <div style={{ position: "relative", width: "100%", maxWidth: 520, maxHeight: "88vh", overflowY: "auto", boxSizing: "border-box", margin: "0 auto", overscrollBehavior: "contain", background: PAPER_DIM, borderTopLeftRadius: 28, borderTopRightRadius: 28, padding: "12px 20px 24px", boxShadow: "0 -20px 40px rgba(0,0,0,0.32)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
            ...
            <div>
              ...
              <p style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, color: MUTED, margin: "2px 0 0" }}>{isEditing ? "Edit spending entry" : "Add a spending entry"}</p>
            </div>
          </div>
        ...
        <div style={{ marginTop: 10, background: INPUT_BG, border: `1px solid ${LINE}`, borderRadius: 16, padding: "13px 14px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
            <span style={{ fontFamily: "Inter, sans-serif", fontSize: 10, color: MUTED, textTransform: "uppercase", fontWeight: 800, letterSpacing: .35 }}>
              Remaining
            </span>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7, marginTop: 7 }}>
          {[["Planned", category.budget, INK], ["Spent", currentSpent, INK]].map(([label, value, color]) => (
            <div key={label} style={{ background: INPUT_BG, borderRadius: 12, padding: "9px 10px" }}>
              <p style={{ fontFamily: "Inter, sans-serif", fontSize: 9.2, color: MUTED, margin: 0 }}>{label}</p>
              <p style={{ fontFamily: "Roboto, sans-serif", fontWeight: 800, fontSize: 11.5, color, margin: "4px 0 0" }}>{formatMoney(value, currency)}</p>
            </div>
          ))}
        </div>
        ...
        <div style={{ marginTop: 9 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
            <span style={{ fontFamily: "Inter, sans-serif", fontSize: 9.5, color: MUTED }}>Category progress</span>
            <span style={{ fontFamily: "Inter, sans-serif", fontSize: 9.5, color: MUTED, fontWeight: 700 }}>
              {category.budget > 0 ? Math.min(100, Math.round((currentSpent / category.budget) * 100)) : 0}%
            </span>
          </div>
          <div style={{ height: 5, background: LINE, borderRadius: 999, overflow: "hidden" }}>
        ...
        <div style={{ marginTop: 9, background: GOLD_GRADIENT, borderRadius: 17, padding: 15, color: "#fff", boxShadow: SHADOW_BTN }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
            <label style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, fontWeight: 800, opacity: 0.85, textTransform: "uppercase", letterSpacing: 0.35 }}>How much did you spend?</label>
        ...
        {!isEditing && (
          <div style={{ display: "flex", gap: 6, marginTop: 9, overflowX: "auto", paddingBottom: 1 }}>
            {quickAmounts.map(({ label, value }) => (
              <button key={`${label}-${value}`} aria-label={`Use ${label} of plan: ${formatMoney(value, currency)}`} onClick={() => setAmount(String(value))} className="pr-press" style={{ border: `1px solid ${LINE}`, background: INPUT_BG, color: INK, borderRadius: 999, padding: "8px 13px", fontFamily: "Roboto, sans-serif", fontWeight: 700, fontSize: 10.5, cursor: "pointer", whiteSpace: "nowrap", transition: "transform 150ms cubic-bezier(0.2,0.8,0.2,1), opacity 150ms ease" }}>{label} · {formatMoney(value, currency)}</button>
            ))}
          </div>
        )}

        {willOver && <div style={{ marginTop: 9, padding: "9px 11px", borderRadius: 11, background: DANGER_BG, border: `1px solid ${CORAL}` }}><p style={{ fontFamily: "Inter, sans-serif", fontWeight: 800, fontSize: 11, color: CORAL, margin: 0 }}>This goes over the category budget.</p><p style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, color: CORAL, margin: "3px 0 0" }}>{formatMoney(overBy, currency)} over after this spending.</p></div>}

        <div style={{ marginTop: 8 }}>
          <label style={{ display: "block", fontFamily: "Inter, sans-serif", fontSize: 9.5, fontWeight: 800, color: MUTED, margin: "0 0 5px 2px", textTransform: "uppercase", letterSpacing: 0.35 }}>Note <span style={{ fontWeight: 600, textTransform: "none", letterSpacing: 0 }}>(optional)</span></label>
        ...
        {!isEditing && transactions.length > 0 ? (
          <div style={{ marginTop: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 7 }}>
              ...
              <span style={{ fontFamily: "Inter, sans-serif", fontSize: 10, color: MUTED }}>{showAllTransactions ? `All ${transactions.length}` : `Latest ${Math.min(4, transactions.length)}`}</span>
            </div>
              ...
              {(showAllTransactions ? transactions : transactions.slice(0, 4)).map((t) => (
                <div key={t.id} style={{ display: "flex", alignItems: "center", gap: 8, background: INPUT_BG, borderRadius: 10, padding: "8px 9px" }}>
                  <button type="button" onClick={() => onStartEdit?.(t)}
                    ...
                    <div style={{ minWidth: 0 }}>
                      <p style={{ fontFamily: "Inter, sans-serif", fontWeight: 700, fontSize: 11, color: INK, margin: 0, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{t.note || "Spending"}</p>
                      <p style={{ fontFamily: "Inter, sans-serif", fontSize: 9.5, color: MUTED, margin: "2px 0 0" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 5, flexShrink: 0 }}>
                    <button type="button" onClick={() => onStartEdit?.(t)} aria-label="Edit spending entry"
                      style={{ border: `1px solid ${LINE}`, background: "transparent", color: INK, borderRadius: 8, minWidth: 48, height: 30, padding: "0 8px", display: "flex", alignItems: "center", justifyContent: "center", gap: 4, cursor: "pointer", fontFamily: "Inter, sans-serif", fontSize: 10, fontWeight: 800 }}>
                      <Pencil size={12} /> Edit
                    </button>
                    <button type="button" onClick={() => { setDeleteTransactionId(t.id); }} aria-label="Delete spending entry"
                      style={{ border: `1px solid ${isDark ? "#633B3B" : "#F0C8C4"}`, background: isDark ? "#2B1B1B" : "#FFF6F5", color: CORAL, borderRadius: 8, minWidth: 54, height: 30, padding: "0 8px", display: "flex", alignItems: "center", justifyContent: "center", gap: 4, cursor: "pointer", fontFamily: "Inter, sans-serif", fontSize: 10, fontWeight: 800 }}>
            ...
            {transactions.length > 4 && (
              <button type="button" onClick={() => setShowAllTransactions((v) => !v)}
                style={{ width: "100%", marginTop: 7, border: `1px solid ${LINE}`, background: INPUT_BG, color: GOLD, borderRadius: 10, padding: "9px 10px", fontFamily: "Inter, sans-serif", fontWeight: 800, fontSize: 10.5, cursor: "pointer" }}>
                {showAllTransactions ? "Show latest 4" : `View all spending (${transactions.length})`}
              </button>
            )}
          </div>
        ) : !isEditing ? (
          <p style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, color: MUTED, textAlign: "center", margin: "14px 0 0" }}>No spending recorded here yet.</p>
        ) : null}
        ...
        <div style={{ position: "sticky", bottom: 0, zIndex: 8, margin: "16px -20px -24px", padding: "10px 20px calc(10px + env(safe-area-inset-bottom))", background: PAPER_DIM, borderTop: `1px solid ${LINE}` }}>
          ...
          {isEditing && (
            <button type="button" onClick={() => setShowDelete(true)} style={{ width: "100%", marginTop: 8, border: "none", background: "transparent", color: CORAL, fontFamily: "Inter, sans-serif", fontWeight: 800, fontSize: 11.5, padding: "5px 8px", cursor: "pointer" }}>
    ...
    <div role="dialog" aria-modal="true" aria-labelledby="backup-sheet-title" aria-describedby="backup-sheet-description" style={{ position: "fixed", inset: 0, zIndex: 9999, display: "flex", alignItems: "flex-end" }}>
      ...
      <div style={{ position: "relative", width: "100%", maxWidth: 520, maxHeight: "min(88vh, 720px)", margin: "0 auto", background: PAPER_DIM, borderTopLeftRadius: 28, borderTopRightRadius: 28, padding: "18px 20px calc(24px + env(safe-area-inset-bottom))", boxShadow: "0 -24px 60px rgba(0,0,0,0.35)", overflowY: "auto", overscrollBehavior: "contain", WebkitOverflowScrolling: "touch" }}>
        ...
        <div style={{ minHeight: 28, marginTop: 9 }}>
          {error && <p style={{ fontFamily: "Inter, sans-serif", fontSize: 11.5, lineHeight: 1.45, color: CORAL, margin: 0 }}>{error}</p>}
          {!error && creating && password.length > 0 && password.length < 8 && <p style={{ fontFamily: "Inter, sans-serif", fontSize: 11.5, color: MUTED, margin: 0 }}>Use at least 8 characters.</p>}
          {!error && creating && confirmPassword.length > 0 && password !== confirmPassword && <p style={{ fontFamily: "Inter, sans-serif", fontSize: 11.5, color: CORAL, margin: 0 }}>Passwords do not match.</p>}
        </div>
...
function HomeScreen({ rules, activeRule, income, setIncome, currency, onSave, onShare, onSelectRule, onEditActive, onHistory, onCurrencyChange, onAddCurrency, historyCount, notes, setNotes, planName, setPlanName, saved, savedSignature, activePlan, onAddExpense, onEditExpense, onDeleteExpense, onFinishPlan, isDark, resourcePrompt, onOpenResource }) {
      ...
      {!activePlan && (
        <section style={{ marginBottom: 16 }}>
          <div style={{
            ...
            boxShadow: "0 16px 34px -24px rgba(11,35,70,0.6)"
          }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
              <div>
                <p style={{ fontFamily: "Inter, sans-serif", fontSize: 9.5, fontWeight: 800, letterSpacing: 1, textTransform: "uppercase", opacity: 0.72, margin: 0 }}>
                  Amount to plan
                </p>
                <button
                  onClick={() => setShowCurrencySheet(true)}
        ...
        <>
          <section>
              ...
              {allocated.map((c, index) => (
                <div key={c.id} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <span style={{ display: "block", fontFamily: "Inter, sans-serif", fontSize: 10.5, color: MUTED, marginTop: 2 }}>
                      {c.pct}% of plan
                    </span>
                  </div>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: INPUT_BG, borderRadius: 13, padding: "10px 12px", marginTop: 8 }}>
              <span style={{ fontFamily: "Inter, sans-serif", fontWeight: 800, fontSize: 11.5, color: MUTED }}>Total</span>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <span style={{ fontFamily: "Inter, sans-serif", fontWeight: 800, fontSize: 10.5, color: isValid ? GOLD : CORAL }}>{total}% allocated</span>
          ...
          <section style={{ marginTop: 14 }}>
            <div style={{
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                <div>
                  <p style={{ fontFamily: "Inter, sans-serif", fontSize: 9.5, fontWeight: 800, letterSpacing: .7, textTransform: "uppercase", color: MUTED, margin: 0 }}>
                    Plan note
                  </p>
                  <p style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, color: MUTED, margin: "3px 0 9px" }}>
          ...
          <section style={{
            ...
            {!isValid && (
              <p style={{ textAlign: "center", fontFamily: "Inter, sans-serif", fontSize: 10.5, color: MUTED, margin: "7px 0 0" }}>
    ...
    <div style={{ paddingBottom: 24 }}>
      {firstRun ? (
        <div style={{ padding: "16px 20px 10px" }}>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
            <p style={{ fontFamily: "Roboto, sans-serif", fontSize: 11, letterSpacing: 1, color: GOLD, fontWeight: 600, margin: 0 }}>STEP 2 OF 2</p>
            <button
      ...
      <div style={{ padding: "0 20px" }}>
        <div style={{ background: PAPER_DIM, border: `1px solid ${LINE}`, borderRadius: 16, padding: 14, boxShadow: "none", marginBottom: 14 }}>
          ...
          />
          {firstRun && (
            <p style={{ fontFamily: "Inter, sans-serif", fontSize: 11.5, color: MUTED, marginTop: 6, marginBottom: 0 }}>
        ...
        {hasDuplicateNames && (
          <div className="pr-shake" style={{ display: "flex", alignItems: "flex-start", gap: 8, background: DANGER_BG, border: `1px solid ${CORAL}`, color: INK, borderRadius: 12, padding: "10px 11px", marginBottom: 10 }}>
            ...
            <div>
              ...
              <p style={{ margin: "3px 0 0", fontFamily: "Inter, sans-serif", fontSize: 11.5, lineHeight: 1.35, color: MUTED }}>
          ...
          {QUICK_CATEGORIES
            ...
            .map((name) => (
              <button
                ...
                style={{ background: INPUT_BG, border: `1px solid ${LINE}`, borderRadius: 999, padding: "4px 9px", color: INK, fontFamily: "Inter, sans-serif", fontWeight: 600, fontSize: 11, cursor: "pointer" }}
              >
                + {name}
              </button>
            ))}
...
function RulesScreen({ rules, activeRuleId, onNew, onOpen, onDuplicate, onDelete }) {
  ...
  return (
    <div style={{ paddingBottom: 24 }}>
      ...
      <p style={{ fontFamily: "Inter, sans-serif", fontSize: 11, color: MUTED, margin: "0 20px 12px" }}>Swipe a Rule left for quick actions.</p>
            ...
            >
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ fontFamily: "Inter, sans-serif", fontSize: 11.5, color: MUTED, margin: "3px 0 0", lineHeight: 1.35 }}>{ruleDescription(r)}</p>
                </div>
                {active && (
                  <span style={{ background: ACTIVE_TINT, color: GOLD, borderRadius: 999, padding: "4px 11px", fontFamily: "Inter, sans-serif", fontWeight: 700, fontSize: 10.5, flexShrink: 0 }}>Default</span>
                )}
  ...
  return (
    <div style={{ paddingBottom: 24 }}>
      <ScreenHeader
        ...
        onBack={onBack}
        right={isActive && (
          <span style={{ background: ACTIVE_TINT, border: `1px solid ${GOLD}`, color: GOLD, borderRadius: 999, padding: "3px 10px", fontFamily: "Inter, sans-serif", fontWeight: 700, fontSize: 10.5, flexShrink: 0 }}>
            Default
          </span>
        )}
      />
      <div style={{ padding: "0 20px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          {!canDelete && (
            <p style={{ textAlign: "center", fontFamily: "Inter, sans-serif", fontSize: 11.5, color: MUTED, margin: 0 }}>
  ...
  return (
    <div style={{ paddingBottom: 24 }}>
      ...
      <div style={{ padding: "0 20px" }}>
        ...
        {sorted.length > 0 && (
          <p style={{ fontFamily: "Inter, sans-serif", fontSize: 11, color: MUTED, margin: "0 0 8px" }}>Swipe an entry left to delete it.</p>
        )}
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {sorted.map((h) => (
            <SwipeRow
            ...
            >
              ...
              >
                <div>
                  ...
                  <p style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, color: MUTED, margin: "1px 0 0" }}>
  ...
  return (
    <div style={{ background: GREEN_CARD_GRADIENT, borderRadius: 26, padding: "26px 22px", color: "#FFFFFF", boxShadow: "0 20px 40px -20px rgba(10,46,27,0.6)" }}>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
        ...
        <p style={{ fontFamily: "Inter, sans-serif", fontWeight: 700, fontSize: 11.5, letterSpacing: 1, textTransform: "uppercase", color: "rgba(255,255,255,0.65)", margin: "18px 0 4px" }}>{ruleName} · Income</p>
      ...
      {options.map((option) => (
        <button key={option.value} type="button" onClick={() => onChange(option.value)}
          style={{ border: "none", borderRadius: 9, padding: "9px 7px", background: value === option.value ? PAPER_DIM : "transparent",
            color: value === option.value ? INK : MUTED, fontFamily: "Inter, sans-serif", fontSize: 11.5, fontWeight: 800,
...
function ReminderPicker({ reminder, setReminder }) {
  ...
  const dayLabel = frequency === "monthly" ? ordinal(dayOfMonth) : day;

  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        <select value={frequency} onChange={(e) => setReminder({ ...reminder, frequency: e.target.value })} aria-label="Reminder frequency"
          style={{ width: "100%", border: `1px solid ${LINE}`, borderRadius: 10, padding: "10px 11px", fontFamily: "Inter, sans-serif", fontSize: 12 }}>
          <option value="off">Off</option><option value="weekly">Weekly</option><option value="monthly">Monthly</option>
        </select>
        {frequency === "weekly" ? (
          <select value={day} onChange={(e) => setReminder({ ...reminder, day: e.target.value })} aria-label="Reminder day"
            style={{ width: "100%", border: `1px solid ${LINE}`, borderRadius: 10, padding: "10px 11px", fontFamily: "Inter, sans-serif", fontSize: 12 }}>
            {WEEKDAYS.map((d) => <option key={d} value={d}>{d}</option>)}
          </select>
        ) : frequency === "monthly" ? (
          <select value={dayOfMonth} onChange={(e) => setReminder({ ...reminder, dayOfMonth: Number(e.target.value) })} aria-label="Reminder date"
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 8 }}>
        <select value={hour} onChange={(e) => setReminder({ ...reminder, hour: Number(e.target.value) })} aria-label="Reminder hour"
          style={{ width: "100%", border: `1px solid ${LINE}`, borderRadius: 10, padding: "10px 11px", fontFamily: "Roboto Mono, monospace", fontSize: 12 }}>
          {Array.from({ length: 12 }, (_, i) => i + 1).map((h) => <option key={h} value={h}>{h}</option>)}
        </select>
        <select value={ampm} onChange={(e) => setReminder({ ...reminder, ampm: e.target.value })} aria-label="Reminder AM or PM"
          style={{ width: "100%", border: `1px solid ${LINE}`, borderRadius: 10, padding: "10px 11px", fontFamily: "Inter, sans-serif", fontSize: 12 }}>
          <option value="AM">AM</option><option value="PM">PM</option>
        </select>
      </div>
      <div style={{ marginTop: 9, display: "flex", alignItems: "center", justifyContent: "center", gap: 10, textAlign: "center" }}>
        {frequency === "off" ? (
          <span style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, color: MUTED }}>Reminders are off.</span>
        ) : (
          <>
            <strong style={{ fontFamily: "Roboto Mono, monospace", fontSize: 13, color: INK }}>{dayLabel}</strong>
            <span style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, color: MUTED }}>{frequency === "weekly" ? "Weekly" : "Monthly"}</span>
            <strong style={{ fontFamily: "Roboto Mono, monospace", fontSize: 13, color: INK }}>{hour} {ampm}</strong>
          </>
        )}
      </div>
    </div>
  ...
  return (
    <div style={{ paddingBottom: 24 }}>
      ...
      <div style={{ padding: "0 20px", display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ background: PAPER_DIM, border: `1px solid ${LINE}`, borderRadius: R_MD, boxShadow: "none", overflow: "hidden" }}>
            ...
            <div style={{ padding: 14, borderBottom: `1px solid ${LINE}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <span style={{ fontFamily: "Inter, sans-serif", fontWeight: 600, fontSize: 13.5, color: INK }}>Allow reminders</span>
              </div>
              <Switch
        ...
        <div>
          ...
          <div style={{ background: PAPER_DIM, border: `1px solid ${LINE}`, borderRadius: R_MD, boxShadow: "none", overflow: "hidden" }}>
            ...
            <div style={{ marginTop: 10, padding: 16, borderRadius: 14, background: BRAND_NAVY, color: "#fff", textAlign: "center" }}>
              <p style={{ fontFamily: "Inter, sans-serif", fontSize: 9.5, fontWeight: 800, letterSpacing: 0.8, textTransform: "uppercase", color: "#A8DDB8", margin: 0 }}>POCKETRULE PRO</p>
              <p style={{ fontFamily: "Sora, sans-serif", fontWeight: 800, fontSize: 16, margin: "5px 0 0" }}>More control. More insight.</p>
              <p style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, color: "rgba(255,255,255,0.68)", margin: "5px auto 0", lineHeight: 1.45, maxWidth: 260 }}>Advanced planning · backups · insights · ad-free</p>
              <div style={{ marginTop: 10 }}>
                <span style={{ display: "inline-flex", background: "rgba(255,255,255,0.1)", color: "#CBEFD4", borderRadius: 999, padding: "5px 11px", fontFamily: "Inter, sans-serif", fontWeight: 700, fontSize: 9.5 }}>Coming soon</span>
              </div>
            </div>
          </div>
        </div>
            ...
            <button onClick={onExportBackup} style={{ width: "100%", background: "none", border: "none", borderBottom: `1px solid ${LINE}`, padding: "13px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div style={{ textAlign: "left" }}>
                  ...
                  <span style={{ display: "block", fontFamily: "Inter, sans-serif", fontSize: 11, color: MUTED, marginTop: 2 }}>Create an encrypted backup with a separate backup password. PocketRule cannot recover a forgotten backup password.</span>
                </div>
              </div>
            ...
            <button onClick={onImportBackup} style={{ width: "100%", background: "none", border: "none", padding: "13px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div style={{ textAlign: "left" }}>
                  ...
                  <span style={{ display: "block", fontFamily: "Inter, sans-serif", fontSize: 11, color: MUTED, marginTop: 2 }}>Restore an encrypted PocketRule backup using its backup password.</span>
                </div>
              </div>
              <ChevronRight size={16} color={MUTED} />
            </button>
          ...
          <div style={{ background: PAPER_DIM, border: `1px solid ${LINE}`, borderRadius: R_MD, boxShadow: "none", overflow: "hidden" }}>
            {[
              ...
              { icon: Shield, label: "Privacy Policy", body: [
                ...
                "10. Contact — Questions about privacy can be sent to support@pocketrule.com."
              ] },
              { icon: ShieldCheck, label: "Terms of Use", body: [
                ...
                "11. Contact — Questions about these terms can be sent to support@pocketrule.com."
              ] },
            ...
            <div style={{ borderTop: `1px solid ${LINE}` }}>
              <button onClick={() => { window.location.href = "mailto:support@pocketrule.com?subject=PocketRule%20Support%20or%20Feature%20Request"; }} style={{ width: "100%", background: "none", border: "none", cursor: "pointer", padding: "13px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}><Mail size={16} color={INK} /><div style={{ textAlign: "left" }}><span style={{ display: "block", fontFamily: "Inter, sans-serif", fontWeight: 600, fontSize: 13.5, color: INK }}>Contact Us / Request a Feature</span><span style={{ display: "block", fontFamily: "Inter, sans-serif", fontSize: 10.5, color: MUTED, marginTop: 2 }}>support@pocketrule.com</span></div></div>
                <ChevronRight size={16} color={MUTED} />
              </button>
            </div>
          </div>
        ...
        <p style={{ textAlign: "center", fontFamily: "Inter, sans-serif", fontSize: 11.5, color: MUTED, marginTop: 4 }}>
    ...
    <div style={{ paddingBottom: 24 }}>
      ...
      <div style={{ padding: "0 20px" }}>
        ...
        <div style={{ marginTop: 16, display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <p style={{ textAlign: "center", fontSize: 11.5, color: MUTED, fontFamily: "Inter, sans-serif" }}>Share opens your device's share sheet — WhatsApp, Instagram, X, and more.</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ height: "100%", overflowY: "auto", padding: "18px 18px 30px", boxSizing: "border-box" }}>
      <div style={{ textAlign: "center" }}>
        ...
        <p style={{ fontFamily: "Inter, sans-serif", fontSize: 11, color: GOLD, fontWeight: 800, letterSpacing: 1.1, margin: 0 }}>RESOURCES</p>
      ...
      <div style={{ background: PAPER_DIM, border: `1px solid ${LINE}`, borderRadius: 20, padding: 18, marginTop: 4, boxShadow: "none", textAlign: "center" }}>
        <p style={{ fontFamily: "Inter, sans-serif", fontSize: 11, color: GOLD, fontWeight: 800, letterSpacing: .9, margin: 0 }}>FEATURED EBOOK</p>
      ...
      {items.map((it) => {
        ...
        return (
          <button key={it.id} onClick={() => onNav(it.id)} style={{ flex: 1, background: "none", border: "none", padding: "6px 0 10px", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, cursor: "pointer", color: isActive ? GOLD : NAV_MUTED }}>
            ...
            <span style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, fontWeight: isActive ? 800 : 600 }}>{it.label}</span>
          </button>
        );
      })}
    </div>
      ...
      <div style={{ padding: "0 20px" }}>
        {active ? (
            ...
            {(() => { const s = planSummary(active); return (
              <>
                <div style={{ textAlign: "center" }}>
                  ...
                  <p style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, fontWeight: 700, letterSpacing: 0.5, textTransform: "uppercase", opacity: 0.7, margin: 0 }}>Active plan</p>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginTop: 18 }}>
                  <div><p style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, opacity: 0.7, margin: 0 }}>Remaining</p><strong style={{ fontFamily: "Roboto, sans-serif", fontSize: 21 }}>{formatMoney(s.remaining, active.currency || currency)}</strong></div>
                  <div style={{ textAlign: "right" }}><p style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, opacity: 0.7, margin: 0 }}>Spent</p><strong style={{ fontFamily: "Roboto, sans-serif", fontSize: 14 }}>{formatMoney(s.spent, active.currency || currency)}</strong></div>
                </div>
          ...
          <div style={{ background: PAPER_DIM, border: `1px dashed ${LINE}`, borderRadius: R_LG, padding: 18, textAlign: "center" }}>
            ...
            <p style={{ fontFamily: "Inter, sans-serif", fontSize: 11.5, color: MUTED, margin: "5px 0 0" }}>Create one from Home when money comes in.</p>
          </div>
        )}
        ...
        <div style={{ margin: "18px 0 0" }}>
          ...
          <div style={{ textAlign: "center" }}>
            ...
            <p style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, color: MUTED, margin: "3px 0 10px" }}>{periodLabel(period)}</p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6, background: PAPER_DIM, border: `1px solid ${LINE}`, borderRadius: 15, padding: 5, marginBottom: 8 }}>
            {[{ id: "week", label: "This week" }, { id: "month", label: "This month" }, { id: "custom", label: "Date range" }].map((item) => (
              <button key={item.id} onClick={() => setPeriod(item.id)} style={{ border: "none", borderRadius: 11, padding: "9px 6px", background: period === item.id ? ACTIVE_TINT : "transparent", color: period === item.id ? GOLD : MUTED, fontFamily: "Inter, sans-serif", fontSize: 10.5, fontWeight: 800, cursor: "pointer" }}>{item.label}</button>
            ))}
          </div>
          ...
          {period === "custom" && (
            <div style={{ background: PAPER_DIM, border: `1px solid ${LINE}`, borderRadius: 14, padding: 10, marginBottom: 10 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                <label style={{ display: "flex", flexDirection: "column", gap: 4, fontFamily: "Inter, sans-serif", fontSize: 9.5, fontWeight: 800, color: MUTED, textTransform: "uppercase" }}>
                  From
              ...
              {!validCustomRange && <p style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, color: CORAL, margin: "7px 0 0", textAlign: "center", fontWeight: 700 }}>Choose an end date on or after the start date.</p>}
            </div>
          )}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 7 }}>
            {[['Planned', planned, INK], ['Spent', totalSpent, INK], ['Left', totalRemaining, GOLD]].map(([label, value, color]) => (
              <div key={label} style={{ background: PAPER_DIM, border: `1px solid ${LINE}`, borderRadius: 14, padding: "11px 8px", textAlign: "center" }}>
                <p style={{ fontFamily: "Inter, sans-serif", fontSize: 9.5, color: MUTED, margin: 0, textTransform: "uppercase", fontWeight: 800, letterSpacing: .3 }}>{label}</p>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {categoryRows.map((c) => (
                  <div key={c.name} style={{ padding: "9px 0", borderTop: `1px solid ${LINE}` }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                      <strong style={{ fontFamily: "Inter, sans-serif", fontSize: 11.5, color: INK }}>{c.name}</strong>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", gap: 8, marginTop: 3 }}>
                      <span style={{ fontFamily: "Inter, sans-serif", fontSize: 9.8, color: MUTED }}>Planned {formatMoney(c.planned, currency)}</span>
                      <span style={{ fontFamily: "Inter, sans-serif", fontSize: 9.8, color: GOLD, fontWeight: 700 }}>{formatMoney(c.remaining, currency)} left</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {categoryRows.length === 0 && (
            <div style={{ background: PAPER_DIM, border: `1px dashed ${LINE}`, borderRadius: 15, padding: 14, textAlign: "center", marginTop: 10 }}>
              <p style={{ fontFamily: "Inter, sans-serif", fontSize: 11, color: MUTED, margin: 0 }}>Your {period === "week" ? "weekly" : period === "month" ? "monthly" : "date-range"} spending insights will appear here after you create a plan and record spending.</p>
            </div>
          )}
          </div>}
        </div>
        ...
        <div style={{ textAlign: "center", margin: "22px 0 10px" }}>
          ...
          <p style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, color: MUTED, margin: "3px 0 0" }}>Tap a plan to view the full breakdown.</p>
        </div>
            ...
            <button key={p.id} onClick={() => onOpenCompleted(p.id)} style={{ width: "100%", textAlign: "left", background: PAPER_DIM, border: `1px solid ${LINE}`, borderRadius: R_MD, padding: 13, marginBottom: 8, boxShadow: "none", cursor: "pointer", color: INK }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div style={{ minWidth: 0, flex: 1 }}>
                  ...
                  <p style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, color: MUTED, margin: "3px 0 0" }}>Completed · {new Date(p.completedAt || p.date).toLocaleDateString()} · {formatMoney(s.budget, p.currency || currency)} planned</p>
                </div>
...
function PocketRuleAppInner() {
  ...
  const [obReminder, setObReminder] = useState({ frequency: "weekly", day: "Friday", dayOfMonth: 1, hour: 12, ampm: "PM" });
  ...
  async function finishOnboarding(selectedRuleId) {
    let notificationsEnabled = false;
    if (obReminder.frequency !== "off") {
      const permission = await requestPocketRuleNotifications();
    ...
    setData((d) => ({
      ...
      settings: {
        ...d.settings,
        reminderFrequency: obReminder.frequency,
        reminderDay: obReminder.day,
        reminderDayOfMonth: obReminder.dayOfMonth,
        reminderHour: obReminder.hour,
        reminderAmpm: obReminder.ampm,
      ...
      <div style={{ width: "min(100%, 780px)", height: "min(780px, calc(100vh - 28px))", minHeight: 0, boxSizing: "border-box", background: PAPER, color: INK, colorScheme: isDark ? "dark" : "light", borderRadius: 34, border: "6px solid #000000", boxShadow: "0 28px 70px rgba(0,0,0,0.52)", overflow: "hidden", display: "flex", flexDirection: "column", position: "relative", ...themeVars }}>
        ...
        <div style={{ flex: 1, overflowY: "auto" }}>
          {!loaded ? (
            ...
            <RuleEditor
              initial={data.rules.find((r) => r.id === editingRuleId) || data.rules[0]}
              firstRun
              onCancel={() => {}}
              onSave={saveRule}
            />
