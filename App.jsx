...
async function ensureExactNotificationAccess({ openSettings = false } = {}) {
  ...
  try {
    ...
    // Capacitor returns the exact-alarm state as `exact_alarm`.
    if (current?.exact_alarm === "granted") {
      return "granted";
    }

    // Never open Android Settings automatically when the app starts.
    // Only do it after the user explicitly enables/saves a reminder.
...
function normalizeSettings(rawSettings, baseSettings) {
  ...
  const rawPin = s.pin;
  const pin =
    (typeof rawPin === "string" && /^\d{4}$/.test(rawPin))
      ? rawPin
      : (rawPin && typeof rawPin === "object" &&
         rawPin.scheme === "pbkdf2-sha256" &&
         Number.isFinite(Number(rawPin.iterations)) && Number(rawPin.iterations) >= 1000 &&
         typeof rawPin.salt === "string" && /^[0-9a-f]+$/i.test(rawPin.salt) && rawPin.salt.length >= 16 &&
         typeof rawPin.hash === "string" && /^[0-9a-f]+$/i.test(rawPin.hash) && rawPin.hash.length === 64)
        ? {
