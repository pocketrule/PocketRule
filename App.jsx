...
const POCKETRULE_REMINDER_BODY = "Your expected money is due today. Open PocketRule and decide where it goes.";
...
async function ensureExactNotificationAccess() {
  if (!isNativeApp()) return "granted";
  try {
    if (typeof LocalNotifications.checkExactNotificationSetting !== "function") return "granted";
    const current = await LocalNotifications.checkExactNotificationSetting();
    if (current?.value === "granted") return "granted";
...
async function schedulePocketRuleReminder(reminder) {
  ...
  const exactAccess = await ensureExactNotificationAccess();
  ...
  try {
    ...
    return result?.notifications?.length ? { ok: true, reason: "scheduled" } : { ok: false, reason: "not-scheduled" };
  } catch (error) {
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: 9, textAlign: "center" }}>
        <span style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, color: MUTED }}>
...
function PocketRuleAppInner() {
  ...
  const [obReminder, setObReminder] = useState({ frequency: "weekly", date: "", day: "Friday", dayOfMonth: 1, hour: 12, ampm: "PM" });
