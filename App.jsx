...
function detectLikelyCurrency() {
  ...
  // Prefer the device/browser region when it is available. This avoids
  // requesting GPS permission during onboarding and still works offline.
  const locales = [];
  try {
  ...
  // Timezone is a useful fallback on devices whose locale does not expose
  // a country (common with custom Android language settings).
  let timezone = "";
  try { timezone = Intl.DateTimeFormat().resolvedOptions().timeZone || ""; } catch {}

  const timezoneToCurrency = [
    [/^Africa\/Lagos$|^Africa\/Porto-Novo$|^Africa\/Accra$/, "NGN"],
    [/^Africa\/Nairobi$/, "KES"],
...
function Onboarding({ reminder, setReminder, onFinish, rules, selectedRuleId, onSelectRule, currency, onCurrencyChange, onAddCurrency }) {
  ...
  useEffect(() => {
    // Currency is normally detected in defaultState/normalizeState. This
    // fallback covers older installs that still have GEN saved.
    if (currency === "GEN") {
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <div style={{ flex: 1, overflowY: "auto", padding: "18px 20px 0" }}>
        <div style={{ textAlign: "center" }}>
          ...
          <p style={{ fontFamily: "Roboto, sans-serif", fontWeight: 600, fontSize: 11.5, color: GOLD, margin: "24px 0 8px" }}>QUICK SETUP</p>
          ...
          <p style={{ fontFamily: "Inter, sans-serif", fontSize: 14, color: MUTED, margin: "12px auto 0", lineHeight: 1.5, maxWidth: 315 }}>
            We’ve set your currency automatically. Pick a starting Rule and you’re ready to plan.
          </p>
        </div>

        <div style={{ margin: "20px auto 0", maxWidth: 330, background: PAPER_DIM, border: `1px solid ${LINE}`, borderRadius: 16, padding: 12, boxShadow: SHADOW_CARD }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
            <div style={{ minWidth: 0 }}>
              <p style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, color: MUTED, margin: "0 0 5px", textTransform: "uppercase", letterSpacing: 0.5, fontWeight: 800, textAlign: "left" }}>Your currency</p>
              <p style={{ fontFamily: "Inter, sans-serif", fontSize: 13, color: INK, margin: 0, fontWeight: 800 }}>
                {currencyLabel(CURRENCIES.find((c) => c.code === currency) || CURRENCIES[0])}
              </p>
              <p style={{ fontFamily: "Inter, sans-serif", fontSize: 10, color: MUTED, margin: "3px 0 0" }}>
            ...
            <button
              ...
              style={{ flexShrink: 0, display: "inline-flex", alignItems: "center", gap: 4, padding: "8px 10px", borderRadius: 10, border: `1px solid ${LINE}`, background: INPUT_BG, color: GOLD, cursor: "pointer", fontFamily: "Inter, sans-serif", fontWeight: 800, fontSize: 11 }}
            >
        ...
        <div style={{ marginTop: 16 }}>
          <p style={{ fontFamily: "Inter, sans-serif", fontWeight: 800, fontSize: 10, color: MUTED, margin: "0 0 8px", textTransform: "uppercase", letterSpacing: .7 }}>Choose your starting Rule</p>
            ...
            {starterRules.map((r) => {
              ...
              return (
                <button key={r.id} onClick={() => onSelectRule(r.id)} style={{ width: "100%", textAlign: "left", display: "flex", alignItems: "center", gap: 12, padding: "13px 14px", borderRadius: 15, border: `1.5px solid ${active ? GOLD : LINE}`, background: active ? ACTIVE_TINT : PAPER_DIM, boxShadow: active ? "0 8px 20px -14px rgba(31,168,74,0.45)" : SHADOW_CARD, cursor: "pointer" }}>
                  <div style={{ width: 38, height: 38, borderRadius: 10, background: active ? "#FFFFFF" : INPUT_BG, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <RuleIcon rule={r} size={19} color={active ? GOLD : INK} />
                  </div>
        ...
        <div style={{ marginTop: 16, background: PAPER_DIM, border: `1px solid ${LINE}`, borderRadius: 16, padding: 14, boxShadow: SHADOW_CARD }}>
          ...
          <p style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, color: MUTED, margin: "9px 2px 0", lineHeight: 1.35 }}>
            You can change reminders anytime in Settings.
          </p>
        </div>

        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, margin: "18px 0 8px", color: MUTED }}>
  ...
  return (
    <div role="dialog" aria-modal="true" aria-labelledby="add-money-title" style={{ position: "fixed", inset: 0, zIndex: 1200, background: "rgba(0,0,0,0.58)", display: "flex", alignItems: "flex-end", justifyContent: "center", padding: 14 }}>
      <div style={{ width: "min(100%, 500px)", maxHeight: "88vh", overflowY: "auto", background: PAPER_DIM, border: `1px solid ${LINE}`, borderRadius: "22px 22px 16px 16px", padding: "18px 16px calc(16px + env(safe-area-inset-bottom))", boxShadow: "0 24px 70px rgba(0,0,0,0.34)" }}>
        ...
        <p style={{ fontFamily: "Inter, sans-serif", fontSize: 11.5, color: MUTED, margin: "5px 0 15px", lineHeight: 1.4 }}>
        ...
        {value > 0 && (
          <div style={{ marginTop: 14, background: ACTIVE_TINT, border: `1px solid ${LINE}`, borderRadius: 14, padding: "11px 12px" }}>
            <p style={{ fontFamily: "Inter, sans-serif", fontSize: 10, fontWeight: 800, color: GOLD, textTransform: "uppercase", letterSpacing: .6, margin: 0 }}>Added to your plan</p>
            <div style={{ marginTop: 7, display: "flex", flexDirection: "column", gap: 5 }}>
              {preview.map((c) => (
                <div key={c.id} style={{ display: "flex", justifyContent: "space-between", gap: 8, fontFamily: "Inter, sans-serif", fontSize: 11.5, color: INK }}>
    ...
    <section style={{ marginTop: 8, paddingBottom: 6 }}>
      <div className="pr-rise" style={{
        ...
        <button
          ...
          style={{
            width: "100%", marginTop: 10, border: `1.5px solid ${GOLD}`, background: ACTIVE_TINT, color: GOLD,
            borderRadius: 12, padding: "10px 12px", fontFamily: "Inter, sans-serif", fontWeight: 800,
            fontSize: 12, cursor: "pointer", display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 7
          }}
        >
          <Plus size={15} strokeWidth={2.8} /> Add money to this plan
        </button>
